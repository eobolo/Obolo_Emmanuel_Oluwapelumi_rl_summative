...
def evaluate_dqn_model(experiment_number):
    """
    Evaluate DQN model and generate graphs.
    
    Args:
        experiment_number (int): The experiment number (1, 2, 3, 4)
    """
    
    # Define file paths
    training_log_file = f"training_log_dqn_{experiment_number}.csv"
    log_path = f"logs/{training_log_file}"
    
    # Check if log file exists
    if not os.path.exists(log_path):
        print(f"ERROR: Log file {log_path} not found!")
        print("Make sure you have trained the DQN model first.")
        return
    ...
    plt.plot(log['timestep'], log['mean_reward_last_100'], 
             label=f'DQN Experiment {experiment_number} - Mean Reward (Last 100 Episodes)',
             linewidth=2, color='blue')
    plt.xlabel('Timestep', fontsize=12)
    plt.ylabel('Mean Reward', fontsize=12)
    plt.title(f'DQN Training Reward Trend - Experiment {experiment_number}', fontsize=14, fontweight='bold')
    plt.legend(fontsize=10)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    
    # Save reward graph
    reward_graph_path = f"graph/dqn_reward_experiment_{experiment_number}.png"
    ...
    plt.plot(log['timestep'], log['mean_episode_length_last_100'], 
             label=f'DQN Experiment {experiment_number} - Mean Episode Length (Last 100 Episodes)',
             linewidth=2, color='green')
    plt.xlabel('Timestep', fontsize=12)
    plt.ylabel('Mean Episode Length', fontsize=12)
    plt.title(f'DQN Training Episode Length Trend - Experiment {experiment_number}', fontsize=14, fontweight='bold')
    plt.legend(fontsize=10)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    
    # Save episode length graph
    episode_graph_path = f"graph/dqn_episode_length_experiment_{experiment_number}.png"
    plt.savefig(episode_graph_path, dpi=300, bbox_inches='tight')
    print(f"Episode length graph saved: {episode_graph_path}")
    plt.show()
    
    # Print summary statistics
    print(f"\n=== DQN Experiment {experiment_number} Summary ===")
...
def evaluate_all_dqn_experiments():
    """Evaluate all DQN experiments (1-4)."""
    print("=== DQN Model Evaluation ===")
    
    for experiment in range(1, 5):
        print(f"\n--- Evaluating DQN Experiment {experiment} ---")
        try:
            evaluate_dqn_model(experiment)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if 1 <= experiment_num <= 5:
            evaluate_dqn_model(experiment_num)
        else:
            print("Experiment number must be between 1 and 4")
    else:
        # Run all experiments
        evaluate_all_dqn_experiments() 